...
import { CasinoBetHistory, PokerBetHistory, SportsBetsV2 } from '../../models';
...
export const list = async (req: Request, res: Response) => {
    ...
    switch (cases) {
        case 'exchange':
            count = await SportsBetsV2.countDocuments(query);
            break;

        case 'poker':
            count = await PokerBetHistory.countDocuments(query);
            break;

        default:
            count = await CasinoBetHistory.countDocuments(query);
            break;
    }
    ...
    if (!pageSize || !page) {
        ...
        switch (cases) {
            case 'exchange':
                results = await SportsBetsV2.aggregate([{ $match: query }, ...aggregateQuery, { $sort: sortQuery }]);
                break;

            case 'poker':
                results = await PokerBetHistory.aggregate([{ $match: query }, ...aggregateQuery, { $sort: sortQuery }]);
                break;

            default:
                results = await CasinoBetHistory.aggregate([{ $match: query }, ...aggregateQuery, { $sort: sortQuery }]);
                break;
        }
        ...
        switch (cases) {
            case 'exchange':
                results = await SportsBetsV2.aggregate(squery);
                break;

            case 'poker':
                results = await PokerBetHistory.aggregate(squery);
                break;

            default:
                results = await CasinoBetHistory.aggregate(squery);
                break;
        };
        return res.json({ results, count });
    }
